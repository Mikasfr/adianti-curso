...
class VendaForm extends TWindow
{
    ...
    function __construct()
    {
        ...
        $cliente_id  = new TDBUniqueSearch('cliente_id', 'local', 'Cliente', 'id', 'nome');
        $obs         = new TText('obs');
        
        // detail fields
        $produto_detail_unqid       = new THidden('produto_detail_uniqid');
        $produto_detail_id          = new THidden('produto_detail_id');
        $produto_detail_produto_id  = new TDBUniqueSearch('produto_detail_produto_id', 'local', 'Produto', 'id', 'descricao');
        ...
        $col_descr->setTransformer(function($value) {
            return Produto::findInTransaction('local', $value)->descricao;
        });
    ...
    public static function onMutationAction($param){
        ...
        TToast::show('info', 'Total: ' . number_format($total, 2, ',', '.'), 'bottom right');
    } 
